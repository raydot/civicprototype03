...
def upgrade():
    # Create openai_usage table for tracking API calls and costs (if it doesn't exist)
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    
    if 'openai_usage' not in inspector.get_table_names():
        op.create_table(
            'openai_usage',
    ...
    # Create cost summary view for quick analytics (drop first if exists)
