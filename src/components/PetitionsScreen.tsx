...
interface PetitionsScreenProps {
  onBack: () => void;
  ...
  return (
    <div className="h-full bg-white flex flex-col relative">
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-4 border-b border-gray-100 bg-white">
        ...
        <h1 className="text-lg font-medium">Petitions</h1>
      ...
      <div className="flex-1 overflow-y-auto pb-24 md:pb-20">
        <div className="p-4">
          <p className="text-gray-600 mb-6 text-center">
            Sign petitions that align with your civic priorities
          </p>

          {/* Petitions List */}
          <div className="space-y-4">
            {petitions.map((petition) => {
              const savedId = `petition-${petition.id}`;
              const isSaved = isRecommendationSaved?.(savedId) || false;
              
              return (
                <div key={petition.id} className="relative">
                  <button
                    onClick={() => onPetitionClick(petition.id)}
